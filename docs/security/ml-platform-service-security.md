# CAB-BOOKING Security Review Workflow ? WORKFLOW 11 — ML-PLATFORM-SERVICE
...
## 14. WORKFLOW 11 — ML-PLATFORM-SERVICE
...
- Preliminary repo-backed concerns:
  - Observed runtime: hiện mới là topology/artifact, chưa có serving/auth/runtime code độc lập.
  - Observed runtime differs from architecture: chưa có runtime service cho feature store / training / serving API / model management.
  - Missing evidence: dataset access policy, secret handling, debug endpoint evidence, serving auth boundary, rollback control.

### Runtime vs Architecture Boundary

- `Observed runtime`
  - `platform/architecture/ai-topology.js`, `platform/ml/feature-store-topology.json`, `platform/node/ai-layer.js` là artifact mô tả scope AI/ML.
  - Chưa thấy `services/ml-platform-service/*`.
  - `platform/node/ai-layer.js` chủ yếu phản chiếu metadata kiến trúc qua header, không phải serving API hoàn chỉnh.
- `Target architecture`
  - Feature Store
  - Model Training
  - Model Serving API
...
- `Observed runtime differs from architecture`
  - chưa có serving API runtime riêng
  - chưa có training pipeline runtime riêng
  - chưa có feature store runtime riêng
...
```text
Bạn là security reviewer cho expected ml-platform-service của CAB-BOOKING.
