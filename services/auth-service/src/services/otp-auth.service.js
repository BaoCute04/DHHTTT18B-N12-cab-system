        ...
        async requestOtp({ role, destination, channel, requestId }) {
            try {
                ...
                try {
                    ...
                    const strictDelivery = Boolean(env && env.otp && env.otp.deliveryRequired);
                    if (strictDelivery) {
                        throw notificationError;
                    }

                    notificationStatus = 'skipped';
                    logger.warn('OTP delivery unavailable, continuing in non-strict mode', {
                        requestId,
                        destination,
                        channel,
                        role,
                        code: notificationError && notificationError.code ? notificationError.code : 'OTP_DELIVERY_UNAVAILABLE',
                        message: notificationError && notificationError.message ? notificationError.message : String(notificationError),
                    });
                }
