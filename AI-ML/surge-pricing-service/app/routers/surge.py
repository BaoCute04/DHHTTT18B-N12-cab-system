...
async def evaluate_surge(payload: SurgeEvaluationRequest):
    try:
        ...
        compatibility_mode = zone_context.get("source") in (
            None, 
            "feature-store-context", 
            "compat-demand-signal", 
            "redis-live-sync-repair",
            "redis-live-initial"
        )
        ...
        # Always call AI directly — do NOT read surge_zone cache.
        # Scheduler writes surge_zone for Kafka/dashboard only, not for quotes.
