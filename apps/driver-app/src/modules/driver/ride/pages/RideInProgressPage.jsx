...
export function RideInProgressPage() {
  ...
  const handleCompleteRide = async () => {
    ...
    try {
      ...
      navigate("/driver/ride/complete", { state: { ride: nextRide } });
    } catch (error) {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-slate-100">
      <div className="w-full max-w-sm h-[760px] bg-white rounded-[28px] shadow-lg overflow-hidden flex flex-col">
        <div className="px-6 pt-6 pb-4 border-b flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold text-slate-900">Ride in progress</h1>
            <p className="text-xs text-slate-500 mt-0.5">Heading to destination</p>
          </div>
        ...
        <div className="px-6 pb-8">
          <div className="rounded-2xl bg-white border shadow-sm p-4 mb-4">
            ...
            <p className="text-xs text-slate-500 truncate">{currentRide?.destination?.address || "Updating..."}</p>
          </div>

          <div className="rounded-2xl bg-slate-50 p-4 mb-4">
            <div className="flex justify-between text-sm mb-2">
              <span>Status</span>
              <span>{currentRide?.status || "IN_PROGRESS"}</span>
            </div>
            <div className="flex justify-between text-sm mb-2">
              <span>ETA</span>
